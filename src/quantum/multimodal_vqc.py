...
def build_exact_ansatz(ansatz_type, num_qubits):
    if ansatz_type == "MPS_TTN":
        print(f"Building hybrid ansatz (Tensor Ring + TTN)")
...
# PIPELINE 
def run_pipeline():
    print(f"STARTING QML PIPELINE (MULTIMODAL DATASET)")
    
    # UPDATED PATH FOR MULTIMODAL DATA
    base_path = r"C:\Users\Utente\Desktop\QuantumSocialRobotics\data\processed\multimodal\quantum_ready"
    
    try:
        print("Loading pre-fused multimodal data")
        ...
        print(f"Files not found in {base_path}. Please check paths.")
        return
    ...
    for name, fm_func, ansatz_type in configurations:
        ...
        try:
            ...
            # VISUALIZATION
            config_plot_dir = os.path.join("plots_multimodal", name)
            ...
            # Training
            print(f"\n Start Training: MaxIter={MAX_ITER}, Classes={NUM_CLASSES}...")
            ...
            # Save Weights 
            np.save(f"multimodal_weights_{name}.npy", vqc.weights)
    ...
    # CSV
    csv_file = "VQC_multimodal_results.csv"
    csv_data = []
    ...
    print(f"\nResults permanently saved in the file: {csv_file}")
