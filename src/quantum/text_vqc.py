...
# PIPELINE
def run_pipeline():
    print(f"STARTING QML PIPELINE (Text Dataset)")
    
    # PATH FOR TEXT
    base_path = r"C:\Users\Utente\Desktop\QuantumSocialRobotics\data\processed\text\quantum_ready"
    ...
    try:
        ...
        print(f"Data successfully loaded. Train: {len(X_train)} | Test: {len(X_test)}")
    ...
    for name, fm_func, ansatz_type in configurations:
        ...
        try:
            ...
            # VISUALIZATION
            config_plot_dir = os.path.join("plots_text", name)
            ...
            # Save Weights
            np.save(f"text_weights_{name}.npy", vqc.weights)
